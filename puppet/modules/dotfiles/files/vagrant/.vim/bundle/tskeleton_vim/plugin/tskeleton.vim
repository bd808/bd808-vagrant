...
augroup tSkeleton
    ...
    if !exists("g:tskelDontSetup") "{{{2
        ...
        " define auto commands for all default template files
        function! s:DefaultAutoCmd()
          let l:files = split(globpath(g:tskelDir,'default.*'),'\n')
          for l:f in l:files
            let l:ext = fnamemodify(l:f, ':e')
            exec 'autocmd BufNewFile' "*.".l:ext 'TSkeletonSetup' l:f
          endfor
        endfunction
        call s:DefaultAutoCmd()
        delfunction s:DefaultAutoCmd
    endif
