...
if !exists('*TSkeleton_YEAR') "{{{2
    function! TSkeleton_YEAR() "{{{3
        return strftime('%Y')
    endf
endif
...
function! tskeleton#TagRx() "{{{3
    ...
    return tskeleton#WrapMarker('\('
            ...
            \ .'\|call:\%(''[^'']*''\|\"\%(\\\"\|[^\"]\)*\"\|[bgs]:\|.\)\{-1,}'
            \ .'\|[a-zA-Z0-9_ -]*\%(/'. text .'\)\?'
            \ .'\|\%(if\|elseif\|for\|input\|select\|let\|\include\|vinclude\|execute\)(.\{-})'
            \ .'\|?.\{-}?'
            \ .'\)\(: *'. text .' *\)\?'
            \ , 'rx')
endf
...
function! s:HandleTag(match, filetype) "{{{3
    ...
    elseif a:match =~# '^vinclude('
        let l:val = s:Var(matchstr(a:match, '(\zs.\{-}\ze)$'))
        return [1, s:Expand(l:val, a:filetype)]
