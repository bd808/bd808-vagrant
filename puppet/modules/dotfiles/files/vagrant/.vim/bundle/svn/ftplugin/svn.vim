...
function! Svn_diff_windows()
  ...
  " find the changed filenames in the current buffer
  while i <= line('$')
    let line = getline(i)
    if line =~ '^[ADMRCXI]'
      let file = substitute(line, '\v^\S*\s*(.*)\s*$', '\1', '')
  ...
  silent! setlocal nosplitbelow nosplitright
  new
  silent! setlocal ft=diff previewwindow bufhidden=delete nobackup noswf nobuflisted nowrap buftype=nofile
  exe "normal :r!svn diff " . list_of_files . "\<CR>"
