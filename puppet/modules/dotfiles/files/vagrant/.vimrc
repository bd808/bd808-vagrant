...
" basic formatting {{{
set shiftwidth=2        " The only sane indent width
...
" encryption {{{
try
  " :X encrypts with blowfix instead of lame ass zip
  set cm=blowfish
catch
endtry
" }}}
...
let g:syntastic_mode_map={ 'mode': 'passive',
      \ 'active_filetypes': ['ruby', 'php', 'perl', 'sh', 'coffee'],
      \ 'passive_filetypes': [] }
...
" tskeleton {{{
let g:tskelDir=$VIMCONF."/data/templates"
let g:tskelBitsDir=$VIMCONF."/data/bits"
...
if has("autocmd")
  ...
  augroup json "{{{
    ...
    autocmd FileType json setlocal noet
  augroup END "}}}
  ...
  augroup vcs "{{{
    au!
    " subversion commit edits
    autocmd BufNewFile,BufRead,BufEnter  svn-commit.* setf SVN
    autocmd BufRead SVN setlocal noautochdir nosb nosr
    autocmd BufRead SVN setlocal spell spelllang=en_us
    autocmd BufEnter SVN call setpos('.', [0, 1, 1, 0])
  ...
  augroup extract "{{{
    au!
    " .tmpl is common extension for expandable templates
    "autocmd BufNewFile,BufRead *.php.tmpl set filetype=php
    ...
    "autocmd BufNewFile,BufRead *.sh.tmpl set filetype=sh
  augroup END "}}}
